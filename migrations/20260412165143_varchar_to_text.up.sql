-- SQLite does not support ALTER COLUMN, so we must recreate each table
-- to change VARCHAR(N) column types to TEXT.
--
-- PRAGMA foreign_keys=off cannot be used inside a transaction (sqlx wraps
-- migrations in one). Instead, new tables reference other _new tables to
-- avoid FK conflicts when dropping originals. SQLite auto-updates FK
-- references when a parent table is renamed (with foreign_keys=ON).
