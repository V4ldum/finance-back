...
mod test {
    ...
    use fake::{faker::lorem::zh_tw::Sentence, Fake};
    use sqlx::{Connection, SqliteConnection};
    use uuid::Uuid;
    ...
    async fn test_params(url: String) -> (ProgramParameters, SqliteConnection) {
    ...
    async fn run_fires_a_request_to_numista_with_proper_headers() {
        ...
        let (params, _db) = test_params(mock_server.address().to_string()).await;
    ...
    async fn run_inserts_the_correct_coin_information_into_the_database() {
        // Arrange
        let mock_server = MockServer::start().await;
        let (params, mut db) = test_params(mock_server.address().to_string()).await;
