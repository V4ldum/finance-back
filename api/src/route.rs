...
use crate::route::cash_assets::{delete_cash_asset, update_cash_asset};
use crate::route::coin_assets::{delete_coin_asset, update_coin_asset};
use crate::route::coins::{get_coin, search_coin};
use crate::route::health_check::health_check;
use crate::route::raw_assets::{delete_raw_asset, update_raw_asset};
...
pub fn router(state: AppState) -> Router {
    ...
    Router::new()
        ...
        .route(
            "/assets/coin/:id",
            get(get_coin_asset)
                .patch(update_coin_asset)
                .delete(delete_coin_asset),
        )
        .route("/assets/raw", post(create_raw_asset))
        .route(
            "/assets/raw/:id",
            get(get_raw_asset)
                .patch(update_raw_asset)
                .delete(delete_raw_asset),
        )
        .route("/assets/cash", post(create_cash_asset))
        .route(
            "/assets/cash/:id",
            get(get_cash_asset)
                .patch(update_cash_asset)
                .delete(delete_cash_asset),
        )
